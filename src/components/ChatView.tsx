...
/**
 * Normalizes Markdown tables and content to ensure tables with missing newlines
 * or glued delimiters render cleanly.
 */
function formatMarkdownContent(rawContent: string): string {
  if (!rawContent) return '';
  let text = rawContent;

  // 1. Fix glued table rows like "...| Catatan ||-----|..." or "...| USD | || 1 |..."
  text = text.replace(/\|\s*\|(?=[-\s\w\d])/g, '|\n|');

  // 2. Fix rows starting with pipe right after a pipe without newline
  text = text.replace(/(\|)\s*(\|)/g, '$1\n$2');

  // 3. Ensure table header separator (|---|---|) has newlines around it
  text = text.replace(/([^\n])\s*(\|(?:\s*[:-]+-+\s*\|)+)/g, '$1\n$2');
                      ...
                      <ReactMarkdown
                        ...
                        components={{
                          table({ children, ...props }: any) {
                            return (
                              <div className="my-3.5 overflow-x-auto rounded-xl border border-white/10 bg-[#121212] shadow-sm">
                                <table className="min-w-full text-left text-xs sm:text-sm border-collapse" {...props}>
                                  {children}
                                </table>
                              </div>
                            );
                          ...
                          thead({ children, ...props }: any) {
                            return (
                              <thead className="bg-[#191919] text-emerald-400 font-semibold border-b border-white/10 uppercase tracking-wider text-[11px]" {...props}>
                                {children}
                              </thead>
                            );
                          },
                          tbody({ children, ...props }: any) {
                            return (
                              <tbody className="divide-y divide-white/5 bg-[#121212]/90 text-slate-200" {...props}>
                                {children}
                              </tbody>
                            );
                          },
                          ...
                          th({ children, ...props }: any) {
                            return (
                              <th className="px-3.5 py-2.5 font-bold text-emerald-300 border-r border-white/5 last:border-r-0 whitespace-nowrap" {...props}>
                                {children}
                              </th>
                            );
                          },
                          td({ children, ...props }: any) {
                            return (
                              <td className="px-3.5 py-2 text-slate-300 border-r border-white/5 last:border-r-0 text-xs sm:text-sm leading-relaxed" {...props}>
                                {children}
                              </td>
                            );
                          },
